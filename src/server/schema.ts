...
export const datasets = pgTable(
  'dataset',
  {
    ...
    name: text().notNull(),
    role: text().notNull(),
    fields: jsonb().$type<Field[]>().notNull(),
    keyField: text('key_field'),
    titleField: text('title_field').notNull(),
    schemaVersion: integer('schema_version').notNull().default(1),
    // Link público do Google Sheets de origem, quando o conjunto veio de um; permite reatualizar.
    sourceUrl: text('source_url'),
    // Sincronização periódica com o link: mapeamento da última importação e resultado da última execução.
    syncEnabled: boolean('sync_enabled').notNull().default(false),
    syncMapping: jsonb('sync_mapping').$type<Mapping>(),
    syncState: jsonb('sync_state').$type<SyncState>(),
  },
  (t) => [index('dataset_project').on(t.projectId)],
);
export const records = pgTable(
  'record',
  {
    ...
    values: jsonb().$type<Record<string, Value>>().notNull(),
...
export const links = pgTable(
  ...
  (t) => [primaryKey({ columns: [t.recordId, t.fieldId] }), index('link_target').on(t.targetId)],
);
...
export const imports = pgTable(
  'import_batch',
  {
    ...
    authorId: text('author_id')
      .notNull()
      .references(() => user.id),
    diagnosis: jsonb().$type<Diagnosis>(),
    mapping: jsonb().$type<Confirmation>(),
    publishedProjectId: uuid('published_project_id').references(() => projects.id),
    requestHash: text('request_hash'),
    sourceUrl: text('source_url'),
    createdAt: time('created_at'),
  },
  (t) => [index('import_workspace').on(t.workspaceId)],
);
